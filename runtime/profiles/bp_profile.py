...
def _run_bp_synthesis_prepare(runtime_root: Path, job_ctx: JobContext) -> dict[str, Any]:
    ...
    synthesis_system_prompt = (
        ...
        '## 专利完整保留规则\n'
        '- 技术维度报告中的专利列表必须完整保留，不做删减\n'
        '- 所有专利的详细描述、覆盖范围、申请状态等信息必须原文呈现\n'
        '- 禁止将详细专利列表压缩为概括性描述（如"另有18项专利覆盖…"等模糊表述）\n'
        '- 专利是技术壁垒的核心证据，完整呈现才能支撑投资判断\n\n'
        '## 技术壁垒量化评估（必须独立成节）\n'
        '- 读者看完报告后必须能明确回答三个问题：\n'
        ...
        '技术原理必须给外行讲透，不能只有术语没有解释。专利部分不做删减，技术维度输出的所有专利内容必须完整保留。\n\n'
        '# 三.5、技术壁垒量化评估\n'
        ...
        '\n'
        '### 规则5：技术维度输出统稿时不做删减（铁律）\n'
        '技术维度子代理（bp_技术与产品）的输出在统稿时**必须完整保留，不做任何删减或压缩**：\n'
        '- 所有技术原理分析、技术路线对比、失效模式分析、核心组件拆解——必须原文保留\n'
        '- 所有专利信息（详细描述、覆盖范围、申请状态）——必须原文保留，不得压缩为概括性描述\n'
        '- 技术壁垒量化评估的完整内容——必须原文保留\n'
        '- 统稿可以对技术内容**重新组织结构/调整顺序/补充交叉引用**，但不得删减、压缩、概括化\n'
        '- 跨维度去重仍可执行（如技术维度和竞争维度都列了同一竞品参数表，保留信息更完整的那个），但技术维度内的任何内容不得删除\n'
        '- **原因**：技术内容删减后反而更难理解，投资人需要完整技术上下文才能判断壁垒\n'
        '\n'
        '### 规则4：来源合并不得丢来源\n'
