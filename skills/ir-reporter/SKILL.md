...
1. 通知推送（告知报告完成 + 文件路径）：
```bash
python3 {IR_RUNTIME}/scripts/notify_plugin.py "📊 {标的名称} 研报已完成。文件位置：{文件路径}"
```
...
**交付链路说明**：
- IR 管线使用 `notify_plugin.py` → 通知插件推送（支持微信/Slack/飞书等）
...
- VL OCR API 配置（代码 default 已内置）：
  ...
  - VL_API_KEY: `YOUR_VL_API_KEY`（小马算力，default 已写死）
